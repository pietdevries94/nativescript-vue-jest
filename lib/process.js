const vueCompiler = require('nativescript-vue-template-compiler')
...
function filterNativeComponents(parts, config) {
  let web = false
  let native = false
  ...
  function checkBlock(block) {
    if (block.attrs === undefined || block.attrs.length === 0) return true
    for (const attr of Object.keys(block.attrs)) {
      if (attr === "web" && block.attrs["web"] !== web) return false
      if (attr === "native" && block.attrs["native"] !== native) return false
    }
    return true
  }
  ...
  parts.templates = parts.templates.filter(checkBlock)
  parts.template = getLast(parts.templates)
  parts.scripts = parts.scripts.filter(checkBlock)
  parts.script = getLast(parts.scripts)
  parts.styles = parts.styles.filter(checkBlock)
  parts.customBlocks = parts.customBlocks.filter(checkBlock)
...
module.exports = function (src, filePath, jestConfig) {
  ...
  parts = filterNativeComponents(parts, vueJestConfig)
